...
// =========================================================
// 【新增】声明定义在 SegPartIntersect.cc 中的切割判定函数
// =========================================================
extern void CheckPrecipitateCutting(Home_t *home); 
// =========================================================
...
static inline int HasNullArms(const Node_t *node)
{
    for (int i=0; i<node->numNbrs; i++) {
        if (node->nbrTag[i].domainID == -1) return(1);
    }
    return(0);
}

static void CompressSparseArmLists(Home_t *home)
{
    for (int i=0; i<home->newNodeKeyPtr; i++) {
        Node_t *node = home->nodeKeys[i];
        if (node == (Node_t *)NULL) continue;
        if (HasNullArms(node)) {
            CompressArmLists(node);
        }
    }
}
...
// =========================================================
    // 【新增】在此处插入切割判定逻辑
    // 逻辑：此时力已经算好了。如果力大于阈值，解锁钉扎节点。
    // 这样接下来的 Integrator 就会让节点动起来。
    // =========================================================
#ifdef ESHELBY
    // 只有在开启 Eshelby 功能时才检查
    CheckPrecipitateCutting(home);
#endif
    // =========================================================
    ...
    VerifyConnections(home, "after CommSendGhosts() in ParadisStep");
