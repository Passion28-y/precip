 ...
 */
                if ((node->numNbrs != 2)                                  ||
                    HAS_ANY_OF_CONSTRAINTS(node->constraint, PINNED_NODE | PRECIPITATE_NODE) ||
...
//            Node_t *node, *nbr1, *nbr2, *mergedNode;
		...
		Node_t *nbr1      = (Node_t *)NULL;
		Node_t *nbr2      = (Node_t *)NULL;
		Node_t *mergedNode= (Node_t *)NULL;
 ...
 //           mergedNode->flags |= NO_MESH_COARSEN;
 	if (mergedNode != (Node_t *)NULL) {
               mergedNode->flags |= NO_MESH_COARSEN;
                ...
                /* MergeNode() 可能因 targetNode orphaned 而把 mergedNode 置空（Topology.cc 里就是这么做的）               这里必须兜底，避免 NULL 解引用；同时保护端点，防止立即再次触发 coarsen。 */
               if (nbr1) nbr1->flags |= NO_MESH_COARSEN;
               if (nbr2) nbr2->flags |= NO_MESH_COARSEN;
               continue;
            }
