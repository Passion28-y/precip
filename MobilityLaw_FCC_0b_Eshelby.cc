                        ...
                        if (numLimits == 0) {
                            ...
                            for (int k=0; k < numLimits; k++) {
                                if ((ratio[1] < ratioList[0][k]) ||
                                    ...
                                    if (ratio[0] < ratioList[0][k]) ratioList[0][k] = ratio[0];
                                    if (ratio[1] > ratioList[1][k]) ratioList[1][k] = ratio[1];
                                }
                            }
 ...
 */
            ...
            // 【新增保护】防止 NaN 导致模拟崩溃
            if (std::isnan(nVel[0]) || std::isnan(nVel[1]) || std::isnan(nVel[2])) {
                nVel[0] = 0.0; nVel[1] = 0.0; nVel[2] = 0.0;
                // 标记该节点力需要重新计算，但不中断模拟
                node->flags |= NODE_RESET_FORCES; 
            }

            // 【新增保护】速度截断：防止非物理的超高速（如超过剪切声速）
            real8 vMax = 1.0e6; // 根据你的单位制设定一个合理上限
            real8 vMag2 = nVel[0]*nVel[0] + nVel[1]*nVel[1] + nVel[2]*nVel[2];
            if (vMag2 > vMax*vMax) {
                real8 scale = vMax / sqrt(vMag2);
                nVel[0] *= scale; nVel[1] *= scale; nVel[2] *= scale;
            }
