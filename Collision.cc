...
#include <vector>
// 声明外部和本地函数
void EllipsoidCollision(Home_t *home);
extern int IntersectionInformation(Home_t *home, real8 pos1[3], real8 pos2[3], EInclusion_t *inclusion, 
                                    real8 newPos1[3], real8 newPos2[3], int *ninc, real8 ratio[2]);
extern int SnapPointToEllipsoidOnPlane(Home_t *home, EInclusion_t *inc, const real8 planePt[3], 
                                        const real8 planeN_in[3], const real8 pin[3], real8 pout[3]);
            ...
            if (Matrix33_Inverse(invMatrix, Matrix) < 0)
{
    int   invOK = 0;
    real8 MatrixReg[3][3];
    real8 trAbs = fabs(Matrix[0][0] + Matrix[1][1] + Matrix[2][2]);
    real8 base  = (trAbs > 1.0) ? trAbs : 1.0;
    real8 regList[4] = {1.0e-12, 1.0e-10, 1.0e-8, 1.0e-6};
    int kk;

    for (kk=0; kk<4; kk++) {
        for (m=0; m<3; m++) for (n=0; n<3; n++) MatrixReg[m][n] = Matrix[m][n];
        MatrixReg[0][0] += regList[kk] * base;
        MatrixReg[1][1] += regList[kk] * base;
        MatrixReg[2][2] += regList[kk] * base;

        if (Matrix33_Inverse(invMatrix, MatrixReg) >= 0) { invOK = 1; break; }
    }

    if (!invOK) {
        /* 兜底：该碰撞点求解不可靠，直接跳过，避免污染几何/拓扑 */
        return(0);
    }
}
...
/* 结果检查：NaN / Inf / 极端大数都直接放弃本次碰撞点 */
if ( (result[0] != result[0]) || (result[1] != result[1]) || (result[2] != result[2]) ||
     (fabs(result[0]) > 1.0e300) || (fabs(result[1]) > 1.0e300) || (fabs(result[2]) > 1.0e300) ) {
    return(0);
}
...
void HandleCollisions(Home_t *home)
{
	if ((home == (Home_t *)NULL) ||
    	...
    	(home->cell2 == (int *)NULL) ||
    	(home->cell2QentArray == (C2Qent_t *)NULL)) {
   	 return;
	}

	EllipsoidCollision(home);
...
/* * 专门处理位错线段与椭球体沉淀的几何碰撞函数 
 * 采用基于 Cell 的邻居搜索，效率远高于全局搜索
 */
void EllipsoidCollision(Home_t *home)
{
#ifdef ESHELBY
    ...
    // 1. 并行检测阶段：只找碰撞，不修改拓扑（保证内存安全）
#pragma omp parallel
    {
        std::vector<CollisionEvent> local_events;
#pragma omp for
        for (int i = 0; i < home->newNodeKeyPtr; i++) {
            Node_t *node1 = home->nodeKeys[i];
            if (node1 == NULL) continue;
            if (node1->myTag.domainID != thisDomain) continue;
            if (HAS_ANY_OF_CONSTRAINTS(node1->constraint, PRECIPITATE_NODE)) continue;

            for (int j = 0; j < node1->numNbrs; j++) {
                Node_t *node2 = GetNeighborNode(home, node1, j);
                if (node2 == NULL) continue;
                if (OrderTags(&node1->myTag, &node2->myTag) >= 0) continue;

                Cell_t *cell = LookupCell(home, node1->cellIdx);
                if (cell == NULL) continue;

                for (int k = 0; k <= cell->nbrCount; k++) {
                    int nbrCellID = (k < cell->nbrCount) ? cell->nbrList[k] : node1->cellIdx;
                    Cell_t *nbrCell = LookupCell(home, nbrCellID);
                    if (nbrCell == NULL) continue;

                    int incIdx = nbrCell->inclusionQ;
                    while (incIdx >= 0) {
                        EInclusion_t *inc = &home->eshelbyInclusions[incIdx];
                        real8 p1[3] = {node1->x, node1->y, node1->z};
                        real8 p2[3] = {node2->x, node2->y, node2->z};
                        PBCPOSITION(param, p1[X], p1[Y], p1[Z], &p2[X], &p2[Y], &p2[Z]);

                        int ninc = 0;
                        real8 ratio[2], nP1[3], nP2[3];
                        if (IntersectionInformation(home, p1, p2, inc, nP1, nP2, &ninc, ratio)) {
                            CollisionEvent ev;
                            ev.tag1 = node1->myTag;
                            ev.tag2 = node2->myTag;
                            ev.inclusionIdx = incIdx;
                            // 计算分割点
                            ev.splitPos[X] = (p1[X]+p2[X])*0.5;
                            ev.splitPos[Y] = (p1[Y]+p2[Y])*0.5;
                            ev.splitPos[Z] = (p1[Z]+p2[Z])*0.5;
                            
                            // 投影到表面
                            real8 planeN[3] = { node1->nx[j], node1->ny[j], node1->nz[j] };
                            real8 snapped[3];
                            if (SnapPointToEllipsoidOnPlane(home, inc, p1, planeN, ev.splitPos, snapped)) {
                                VECTOR_COPY(ev.splitPos, snapped);
                            }
                            local_events.push_back(ev);
                        }
                        incIdx = inc->nextInCell;
                    }
                }
            }
        }
#pragma omp critical
    ...
    // 2. 串行处理阶段：统一执行 SplitNode（绝对安全）
    for (auto &ev : events) {
        Node_t *node1 = GetNodeFromTag(home, ev.tag1);
        Node_t *node2 = GetNodeFromTag(home, ev.tag2);
        if (!node1 || !node2) continue;

        // 再次检查约束，避免重复分裂
        if (HAS_ANY_OF_CONSTRAINTS(node1->constraint, PRECIPITATE_NODE)) continue;

        Node_t *sNode1, *sNode2;
        int arm21 = GetArmID(node2, node1);
        real8 pos2[3] = {node2->x, node2->y, node2->z};
        PBCPOSITION(param, node1->x, node1->y, node1->z, &pos2[X], &pos2[Y], &pos2[Z]);

        if (SplitNode(home, OPCLASS_COLLISION, node2, pos2, ev.splitPos, &node2->vX, &node2->vX, 1, &arm21, 1, &sNode1, &sNode2, 0) == SPLIT_SUCCESS) {
            if (sNode1) { sNode1->constraint |= (PRECIPITATE_NODE | SURFACE_NODE); MarkNodeForceObsolete(home, sNode1); }
            if (sNode2) { sNode2->constraint |= (PRECIPITATE_NODE | SURFACE_NODE); MarkNodeForceObsolete(home, sNode2); }
        }
    }
#endif
}