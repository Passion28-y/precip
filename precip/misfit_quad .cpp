...
static inline void voigtToMat(const std::array<double,6>& v, double s[3][3]){
    s[0][0]=v[0]; s[1][1]=v[1]; s[2][2]=v[2];
    s[1][2]=s[2][1]=v[3]; s[0][2]=s[2][0]=v[4]; s[0][1]=s[1][0]=v[5];
}
                         ...
                         std::array<double,6>& sigma_voigt)
{
    ...
    for(int it=0; it<Ntheta; ++it){
        ...
        for(int ip=0; ip<Nphi; ++ip){
            ...
            // area element Jacobian on ellipsoid surface
            ...
            double w = (sph) * dth * dph; // surface element weight on sphere, scaled by J approx
...
#if 0
void nodeForceFromMisfitOnSegment(const std::array<double,3>& x1,
                                  const std::array<double,3>& x2,
                                  const std::array<double,3>& b,
                                  const std::array<double,3>& xi,
                                  const std::function<void(const std::array<double,3>&, std::array<double,6>&)>& sigma_at,
                                  std::array<double,3>& f1_out)
#endif
{
    // 2-pt Gauss on s∈[0,1]: points {0.5±1/2√(1/3)}, weights 0.5
    const double s1 = 0.5-0.5/std::sqrt(3.0);
    const double s2 = 0.5+0.5/std::sqrt(3.0);
    const double w  = 0.5;
    std::array<double,3> pts[2] = {
        { x1[0] + s1*(x2[0]-x1[0]), x1[1] + s1*(x2[1]-x1[1]), x1[2] + s1*(x2[2]-x1[2]) },
        { x1[0] + s2*(x2[0]-x1[0]), x1[1] + s2*(x2[1]-x1[1]), x1[2] + s2*(x2[2]-x1[2]) }
    };
    std::array<double,3> acc = {0,0,0};
    for(int k=0;k<2;k++){
        std::array<double,6> sv;
        sigma_at(pts[k], sv);
        double S[3][3]; voigtToMat(sv, S);
        // σ·b
        double sb[3] = { S[0][0]*b[0]+S[0][1]*b[1]+S[0][2]*b[2],
        ...
        acc[0] += w*fk[0]; acc[1] += w*fk[1]; acc[2] += w*fk[2];
    }
    f1_out = acc;
}