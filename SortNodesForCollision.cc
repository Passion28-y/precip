 ...
 */
if (numCell2s <= 0) {
    numCell2s = 1;  /* 兜底：避免 realloc(ptr,0) */
}

/* 推荐写法：用临时指针接住 realloc 的返回值 */
int *tmp = (int *)realloc(home->cell2, numCell2s * sizeof(int));
if (tmp == (int *)NULL) {
    Fatal("%s::%s(%d) realloc(home->cell2) failed\n", __FILE__, __func__, __LINE__);
}
home->cell2 = tmp;
...
#pragma omp parallel for
        ...
        int qentAlloc = (home->newNodeKeyPtr > 0) ? home->newNodeKeyPtr : 1;

home->cell2QentArray = (C2Qent_t *)realloc(home->cell2QentArray,
                                           qentAlloc *
                                           sizeof(C2Qent_t));

if (home->newNodeKeyPtr <= 0) {
    home->cell2QentArray[0].node = (Node_t *)NULL;
    home->cell2QentArray[0].next = -1;
}
